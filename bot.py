...
BOT_TOKEN = "8494602735:AAGbzBwtrk1ZycDpubMjOVRhGQWKivQYzzU" # Замени на свой токен, если нужно
...
def _load_data_sync(filename):
    if os.path.exists(filename):
        try:
            ...
            # ЗАЩИТА ОТ КРАША: Если файл сломался из-за сбоя хостинга, бэкапим его и отдаем пустой словарь
...
async def train_execute_handler(callback: CallbackQuery):
    ...
    rating_chance = 0.35 if callback.data.split(":")[1] in ["tech", "shoot"] else 0.20
    current_rating = p.get("rating", 40)
    
    if current_rating >= 95: rating_chance *= 0.05
    elif current_rating >= 90: rating_chance *= 0.15
    elif current_rating >= 85: rating_chance *= 0.30
    elif current_rating >= 75: rating_chance *= 0.60
    
    if random.random() < rating_chance:
        p["rating"] = min(100, p["rating"] + 1)
 
    players[user_id] = p
    await save_data(PLAYERS_FILE, players)
    await callback.message.delete()
    await callback.message.answer(text="💪 **Тренировка завершена!** Доверие повышено.", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🏠 Меню", callback_data="back_to_menu")]]))
...
async def profile_handler(callback: CallbackQuery):
    ...
    season_display = min(p['season'], 13) # Изменено до 13
...
async def sponsors_menu(callback: CallbackQuery):
    ...
    if callback.message.photo:
        await callback.message.delete()
        await callback.message.answer("💰 **Рекламные контракты**", reply_markup=kb, parse_mode="Markdown")
    else:
        await callback.message.edit_text("💰 **Рекламные контракты**", reply_markup=kb, parse_mode="Markdown")
...
async def sponsor_sign(callback: CallbackQuery):
    ...
    if callback.message.photo:
        await callback.message.delete()
        await callback.message.answer(f"🤝 **Контракт с {sp} подписан!**", parse_mode="Markdown", reply_markup=await main_menu_keyboard(callback.from_user.username, user_id))
    else:
        await callback.message.edit_text(f"🤝 **Контракт с {sp} подписан!**", parse_mode="Markdown", reply_markup=await main_menu_keyboard(callback.from_user.username, user_id))
...
async def match_handler(callback: CallbackQuery, state: FSMContext):
    ...
    # --- СБОРНАЯ (ЧМ / ЕВРО / КОПА) ---
    if p["tour"] > 15:
        if p["rating"] >= 78:
            is_wc = p["season"] in [1, 4, 8, 12]
            is_euro_copa = p["season"] in [3, 7, 11]
            
            if is_wc or is_euro_copa:
                if p.get("wc_out") or p.get("wc_stage") == "Победитель":
                    return await season_results_handler(callback)
                
                tourney_name = "ЧЕМПИОНАТ МИРА" if is_wc else "ЕВРО" if p.get("nation", "Россия") in EURO_NATIONS else "КОПА АМЕРИКА"
                
                if not p.get("wc_rivals"):
                    if is_wc: pool = [n for n in NATIONS if n != p.get("nation", "Россия")]
                    elif tourney_name == "ЕВРО": pool = [n for n in EURO_NATIONS if n != p.get("nation", "Россия")]
                    else: pool = [n for n in COPA_NATIONS if n != p.get("nation", "Россия")]
                    
                    if len(pool) < 3: pool = NATIONS
                    p["wc_rivals"] = random.sample(pool, 3)
                    p["wc_stage"] = "Группа 1 матч" if is_wc else "1/8 финала"
                    p["wc_points"] = 0
                    p["wc_table"] = [
                        {"name": p.get("nation", "Россия"), "points": 0, "games": 0},
                        {"name": p["wc_rivals"][0], "points": 0, "games": 0},
                        {"name": p["wc_rivals"][1], "points": 0, "games": 0},
                        {"name": p["wc_rivals"][2], "points": 0, "games": 0}
                    ]
                    players[user_id] = p
                    await save_data(PLAYERS_FILE, players)
                    
                curr_stage = p.get("wc_stage", "Группа 1 матч")
                if curr_stage.startswith("Группа"):
                    match_idx = int(curr_stage.split()[1]) - 1
                    rival = p["wc_rivals"][match_idx]
                else:
                    if is_wc: pool = [n for n in NATIONS if n != p.get("nation", "Россия") and n not in p.get("wc_rivals", [])]
                    elif tourney_name == "ЕВРО": pool = [n for n in EURO_NATIONS if n != p.get("nation")]
                    else: pool = [n for n in COPA_NATIONS if n != p.get("nation")]
                    rival = random.choice(pool) if pool else random.choice(NATIONS)
                    
                match_data = {
                    "rival": rival, "total_moments": random.randint(1, 4),
                    "current_moment": 1, "minute": 0, "goals": 0, "assists": 0,
                    "saves": 0, "tackles": 0, "yellow_cards": 0, "my_team_score": 0, "rival_team_score": 0,
                    "is_cup": True, "is_national": True, "wc_stage": curr_stage, "log": "", "tourney": tourney_name
                }
                title = f"🏆 {tourney_name}: {curr_stage} 🏆"
                await state.update_data(match=match_data)
                
                if callback.message.photo: await callback.message.delete()
                
                msg = await callback.message.answer(f"🌐 **{title}**\n⚔️ **{p.get('nation', 'Россия')}** vs **{match_data['rival']}**\nСудья дает свисток!", parse_mode="Markdown")
                await asyncio.sleep(2)
                await msg.delete()
                return await generate_moment(callback, state, user_id)
            else:
                played = p.get("nat_played", 0)
                if played < 2:
                    p["nat_played"] = played + 1
                    players[user_id] = p
                    await save_data(PLAYERS_FILE, players)
                    
                    rival_nation = random.choice([n for n in NATIONS if n != p.get("nation", "Россия")])
                    match_data = {
                        "rival": rival_nation, "total_moments": random.randint(1, 4),
                        "current_moment": 1, "minute": 0, "goals": 0, "assists": 0, "saves": 0, "tackles": 0,
                        "yellow_cards": 0, "my_team_score": 0, "rival_team_score": 0,
                        "is_cup": False, "is_national": True, "log": ""
                    }
                    await state.update_data(match=match_data)
                    
                    if callback.message.photo: await callback.message.delete()
                    
                    msg = await callback.message.answer(f"🌐 **🤝 ТОВАРИЩЕСКИЙ МАТЧ СБОРНЫХ**\n⚔️ **{p.get('nation', 'Россия')}** vs **{match_data['rival']}**\nСудья дает свисток!", parse_mode="Markdown")
                    await asyncio.sleep(2)
                    await msg.delete()
                    return await generate_moment(callback, state, user_id)
                else:
...
async def generate_moment(callback: CallbackQuery, state: FSMContext, user_id: str):
    ...
    is_nat = m.get("is_national", False)
    ...
    if m["current_moment"] > m["total_moments"] or m["minute"] == 90:
        ...
        if m["is_cup"]:
            if m.get("is_national"):
                is_knockout = not m.get("wc_stage", "").startswith("Группа")
            else:
                is_knockout = True
...
async def gk_action_handler(callback: CallbackQuery, state: FSMContext):
    ...
    rival_rating = NATION_RATINGS.get(m["rival"], 75) if m.get("is_national") else CLUB_RATINGS.get(m["rival"], 50)
...
async def act_pass_handler(callback: CallbackQuery, state: FSMContext):
    ...
    rival_rating = NATION_RATINGS.get(m["rival"], 75) if m.get("is_national") else CLUB_RATINGS.get(m["rival"], 50)
    pass_chance = 0.25 + ((p["rating"] - rival_rating) * 0.01) + (p["rating"] * 0.003)
    pass_chance = max(0.1, min(0.85, pass_chance))
    
    if random.random() < pass_chance:
        m["assists"] += 1; m["my_team_score"] += 1
        await callback.message.delete()
        tmp_msg = await callback.message.answer(text=f"🅰️ **{m['minute']}'** | Роскошный проникающий пас! Партнер замыкает передачу в касание!", parse_mode="Markdown")
        await asyncio.sleep(2)
        await tmp_msg.delete()
        m["log"] += f"🅰️ **{m['minute']}'** | Твой роскошный голевой пас!\n"
    else:
...
async def start_penalty_shootout(callback: CallbackQuery, state: FSMContext, user_id: str):
    ...
    try:
        await callback.message.edit_text("⏱ **Основное время завершилось вничью!**\n🏆 Начинается **Серия Послематчевых Пенальти**!", parse_mode="Markdown")
    except:
        await callback.message.answer("⏱ **Основное время завершилось вничью!**\n🏆 Начинается **Серия Послематчевых Пенальти**!", parse_mode="Markdown")
...
async def next_penalty_kick(callback: CallbackQuery, state: FSMContext, user_id: str):
    ...
    if m["pen_round"] > 5 and m["my_pen_score"] != m["rival_pen_score"]:
        await finish_cup_match(callback, state, user_id)
        return
 
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎯 Налево", callback_data="pen_dir:left"), InlineKeyboardButton(text="🎯 По центру", callback_data="pen_dir:center"), InlineKeyboardButton(text="🎯 Направо", callback_data="pen_dir:right")]
    ])
    
    try:
        await callback.message.edit_text(
            f"🥅 **СЕРИЯ ПЕНАЛЬТИ — Раунд {m['pen_round']}**\n"
            f"Счет по пенальти: **{m['my_pen_score']} : {m['rival_pen_score']}**\n\n"
            f"Выбирай направление для своего удара/сейва:", reply_markup=kb
        )
    except:
        await callback.message.answer(
            f"🥅 **СЕРИЯ ПЕНАЛЬТИ — Раунд {m['pen_round']}**\n"
...
async def execute_penalty_kick(callback: CallbackQuery, state: FSMContext):
    ...
    gk_action = random.choice(["left", "center", "right"])
    if choice != gk_action:
        m["my_pen_score"] += 1
        my_res = "✅ Ты забил пенальти!"
    else:
        my_res = "❌ Твой удар парировал вратарь!"
        
    rival_choice = random.choice(["left", "center", "right"])
    my_gk_dive = random.choice(["left", "center", "right"])
    
    if rival_choice != my_gk_dive:
        m["rival_pen_score"] += 1
        rival_res = "⚽ Соперник забивает точным ударом."
    else:
        rival_res = "🧤 ТЫ ОТРАЗИЛ ПЕНАЛЬТИ СОПЕРНИКА!"
    ...
    try:
        await callback.message.edit_text(f"📊 **Итоги раунда:**\n{m['log']}")
    except:
        await callback.message.answer(f"📊 **Итоги раунда:**\n{m['log']}")
        
    await asyncio.sleep(2)
    await next_penalty_kick(callback, state, user_id)
 
async def finish_cup_match(callback: CallbackQuery, state: FSMContext, user_id: str):
    data = await state.get_data()
    if "match" not in data:
        return
    m = data["match"]
    is_win = m["my_pen_score"] > m["rival_pen_score"]
    
    if is_win:
        m["my_team_score"] += 1 
        status_text = "\n🎉 ПОБЕДА ПО ПЕНАЛЬТИ! Вы проходите дальше!"
    else:
        m["rival_team_score"] += 1
        status_text = "\n❌ Поражение по пенальти. Команда вылетает."
        
    await state.update_data(match=m)
    await finish_match(callback, state, user_id, cup_status=status_text)

# --- ЗАВЕРШЕНИЕ МАТЧА И ИНТЕРВЬЮ ---
async def finish_match(callback: CallbackQuery, state: FSMContext, user_id: str, cup_status: str = ""):
    ...
    if not p or not m:
        return await callback.answer("Ошибка данных матча.", show_alert=True)
        
    my_score = m.get("my_team_score", 0)
    rival_score = m.get("rival_team_score", 0)
    
    # Обновляем статистику
    p["stats_season"]["goals"] = p["stats_season"].get("goals", 0) + m.get("goals", 0)
    p["stats_season"]["assists"] = p["stats_season"].get("assists", 0) + m.get("assists", 0)
    p["stats_season"]["saves"] = p["stats_season"].get("saves", 0) + m.get("saves", 0)
    p["stats_season"]["tackles"] = p["stats_season"].get("tackles", 0) + m.get("tackles", 0)
    
    if "stats_total" not in p:
        p["stats_total"] = {"games": 0, "goals": 0, "assists": 0, "saves": 0, "tackles": 0}
    ...
    # Логика кубков и турниров
    if m.get("is_cup") and not m.get("is_national"):
        if outcome == "loss":
            p["cup_out"] = True
        else:
            stages = CUP_STAGES
            ...
            if curr_idx < len(stages) - 1:
                ...
                cup_status += "\n🏆 ВЫ ВЫИГРАЛИ КУБОК!"
                p["trust"] = 100
    elif m.get("is_national"):
        if m.get("tourney") in ["ЧЕМПИОНАТ МИРА", "ЕВРО", "КОПА АМЕРИКА"]:
            if outcome == "win": p["wc_points"] = p.get("wc_points", 0) + 3
            elif outcome == "draw": p["wc_points"] = p.get("wc_points", 0) + 1
    
    players[user_id] = p
    await save_data(PLAYERS_FILE, players)
    
    if not m.get("is_cup") and not m.get("is_national"):
...
@dp.callback_query(F.data == "season_results")
async def season_results_handler(callback: CallbackQuery):
    ...
    # Проверка на пенсию (13 сезонов)
    if p["season"] > 13:
        p["retired"] = True
        if "trophies" not in p: p["trophies"] = []
        await add_to_retired_leaderboard(p["name"], p["rating"], len(p["trophies"]))
        hist_str = f"Клуб: {p['club']} | Сезонов: {p['season']-1} | Рейтинг: {p['rating']} | Трофеев: {len(p['trophies'])}"
        
        if "career_history" not in p: p["career_history"] = []
        p["career_history"].append(hist_str)
        
        players[user_id] = p
        await save_data(PLAYERS_FILE, players)
        
        text = "🏁 **Твоя карьера подошла к концу!**\nТы провел 13 сезонов на высшем уровне и стал настоящей легендой. Твой профиль сохранен в Зале Славы."
        try:
            await callback.message.edit_text(text, parse_mode="Markdown", reply_markup=retired_keyboard())
        except:
            await callback.message.answer(text, parse_mode="Markdown", reply_markup=retired_keyboard())
    ...
    text = f"🎉 **СЕЗОН ЗАВЕРШЕН!**\nТебе теперь {p['age']} лет. Начинается новый {p['season']}-й сезон! Статистика обнулена, готовься к новым победам."
    kb = await main_menu_keyboard(callback.from_user.username, user_id)
